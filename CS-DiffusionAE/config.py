    ...
    def make_dataset(self, path=None, **kwargs):
        if self.data_name == 'ffhqlmdb256':
            ...
            return CelebAlmdb(path=path or self.data_path,
                              ...
                              crop_d2c=True,
                              **kwargs)
        # elif self.data_name == 'ffhqlmdb256_mlp':
        #     return FFHQlmdb_mlp(path=path or self.data_path,
        #         image_size=self.img_size,
        #         **kwargs)
        else:
            raise NotImplementedError()

    def make_mlp_dataset(self, path_bg=None, path_t=None, **kwargs):
        
        dataset_bg = FFHQlmdb_mlp(path=path_bg,
            image_size=self.img_size,
            **kwargs)
        
        dataset_t = FFHQlmdb_mlp(path=path_t,
            image_size=self.img_size,
            **kwargs)
        
        return dataset_bg, dataset_t
