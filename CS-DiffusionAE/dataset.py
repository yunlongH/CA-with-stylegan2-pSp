...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {'img': img, 'index': index, 'filename': filename}  # Return filename as well

class SingleImageDataset(Dataset):
    def __init__(self, image_path, image_size, do_augment=True, do_transform=True, do_normalize=True):
        super().__init__()
    ...
    def __getitem__(self, index):
        ...
        img = img.convert('RGB')  # Ensure RGB format
        filename = os.path.basename(self.image_path)  # Extract filename

        if self.transform is not None:
            img = self.transform(img)

        return {'img': img, 'index': 0, 'filename': filename}  # Always index 0 since it's one image


class LMDBDataset(Dataset):
    def __init__(self, lmdb_path, load_image=True):
        self.env = lmdb.open(lmdb_path, readonly=True, lock=False, readahead=False, meminit=False)
        self.load_image = load_image

        # Path to cached key list
        cache_path = os.path.join(lmdb_path, "keys_cache.pkl")
        ...
        if os.path.exists(cache_path):
            ...
            # First time: load from LMDB and cache to disk
    ...
    def __getitem__(self, index):
        ...
        result = {
            'latent': torch.tensor(entry['latent'], dtype=torch.float32).squeeze(0),
            'filename': entry['filename']
        }

        if self.load_image:
            result['img'] = torch.tensor(entry['img'], dtype=torch.float32)
...
class ImageDatasetFromList(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {'img': img, 'index': index, 'filename': filename}

class LatentDataset(Dataset):
    def __init__(self, hdf5_file_bg, hdf5_file_glass):
        self.bg_h5 = h5py.File(hdf5_file_bg, "r")
        self.glass_h5 = h5py.File(hdf5_file_glass, "r")

        # Load filenames
        self.bg_filenames = [name.decode() for name in self.bg_h5["filenames"][:]]
        self.glass_filenames = [name.decode() for name in self.glass_h5["filenames"][:]]

        self.bg_latents = self.bg_h5["encodings"]
        self.glass_latents = self.glass_h5["encodings"]

    def __len__(self):
        return min(len(self.bg_filenames), len(self.glass_filenames))  # Ensure equal pairing

    def __getitem__(self, index):
        filename_bg = self.bg_filenames[index]
        filename_t = self.glass_filenames[index]

        # Load corresponding latent vectors and remove extra dimension
        latent_bg = torch.from_numpy(np.array(self.bg_latents[filename_bg])).float().squeeze(0)  # Remove extra dim
        latent_t = torch.from_numpy(np.array(self.glass_latents[filename_t])).float().squeeze(0)  # Remove extra dim

        return {
            "diffAE_zbg": latent_bg,
            "diffAE_zt": latent_t,
...
class BaseLMDB(Dataset):
    def __init__(self, path, original_resolution, zfill: int = 5):
        ...
        with self.env.begin(write=False) as txn:
            cursor = txn.cursor()
            for key, _ in cursor:
                if key != b'length':  # Ignore the 'length' key
                    self.keys.append(key.decode('utf-8'))

        self.keys = sorted(self.keys)  # Ensure consistent ordering
        self.length = len(self.keys)
    ...
    def __getitem__(self, index):
        with self.env.begin(write=False) as txn:
            key = self.keys[index].encode('utf-8')  # Use preloaded keys
            img_bytes = txn.get(key)
            ...
            try:
                ...
                raise ValueError(f"Error loading image at key {key}: {e}")
...
class FFHQlmdb_mlp(Dataset):
    def __init__(self,
                 path=os.path.expanduser('datasets/ffhq256_mlp.lmdb'),
                 image_size=256,
                 original_resolution=256,
                 as_tensor: bool = True,
                 do_augment: bool = True,
