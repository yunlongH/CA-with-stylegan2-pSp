...
def train(cs_model, diff_model, optimizer, 
          disc_bg_model, disc_bg_optimizer, disc_t_model, disc_t_optimizer, 
    ...
    for epoch in range(args.max_epochs):
        ...
        train_D_dict = {}  # discriminator loss dict
        disc_updates = 0
        ...
        for batch_idx, (batch_bg, batch_t) in enumerate(zip(trainloader_bg, trainloader_t)):
            ...
            if batch_idx % args.disc_iter_interval == 0:
                loss_D_bg = train_disc_step(latent_real=diffAE_zbg, latent_fake=c_t,
                                            model=disc_bg_model, optimizer=disc_bg_optimizer,
                                            criterion=criterion_disc)

                loss_D_t = train_disc_step(latent_real=diffAE_zt, latent_fake=c_bg + s_t,
                                           model=disc_t_model, optimizer=disc_t_optimizer,
                                           criterion=criterion_disc)

                # Accumulate discriminator losses
                for loss_name, loss_tensor in loss_D_bg.items():
                    name = f"{loss_name}_bg"
                    train_D_dict.setdefault(name, 0)
                    train_D_dict[name] += loss_tensor.item()

                for loss_name, loss_tensor in loss_D_t.items():
                    name = f"{loss_name}_t"
                    train_D_dict.setdefault(name, 0)
                    train_D_dict[name] += loss_tensor.item()
            ...
            adv_loss_bg = calc_adv_loss(latent_fake=c_t, model=disc_bg_model, criterion=criterion_disc)
            adv_loss_t = calc_adv_loss(latent_fake=c_bg + s_t, model=disc_t_model, criterion=criterion_disc)
            adv_loss = args.w_adv * (adv_loss_bg + adv_loss_t)
            ...
            # Accumulate latent and adversarial losses
            for loss_name, loss_value in latent_loss_list.items():
                train_dict.setdefault(loss_name, 0)
                train_dict[loss_name] += loss_value.item()

            train_dict.setdefault("loss_adv_bg", 0)
            train_dict["loss_adv_bg"] += adv_loss_bg.item()
            train_dict.setdefault("loss_adv_t", 0)
            train_dict["loss_adv_t"] += adv_loss_t.item()
            train_dict["total_loss"] += total_batch_loss.item()

        # Average generator losses
...
def main():
    ...
    disc_bg_model, disc_bg_optimizer = load_disc_model(device, args, is_train=True)
    disc_t_model, disc_t_optimizer = load_disc_model(device, args, is_train=True)
