...
from custom_funcs.loss_funcs import calc_latent_recon_loss

def train(cs_model, diff_model, optimizer, trainloader_bg, trainloader_t, valloader_bg, valloader_t, test_image_fixed, args, device):
    ...
    for epoch in range(args.max_epochs):  # Iterate over epochs
        ...
        for batch_idx, (batch_bg, batch_t) in enumerate(zip(trainloader_bg, trainloader_t)):
            # imgs_bg = batch_bg['img'].to(device, non_blocking=True)
            # imgs_t = batch_t['img'].to(device, non_blocking=True)
        ...
        if (epoch + 1) % args.image_interval == 0:
            ...
            #test_image_fixed = get_fixed_for_test(batch_bg, batch_t, diff_model, device, T_noise=args.T_noise, T_render=args.T_noise, idx=1)
...
def main():
    ...
    #parser.add_argument('--calc_image_loss', type=bool, default=False)
