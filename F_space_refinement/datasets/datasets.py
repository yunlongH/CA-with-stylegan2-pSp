...
import os
import numpy as np
# from data_utils import make_dataset  # adjust if make_dataset is from elsewhere
...
class ImageDataset_pathlist(Dataset):
    def __init__(self, root_or_paths, transform=None):
        """
        Args:
            root_or_paths (str or list): Directory path or list of image paths.
            transform: torchvision transforms to apply to each image.
        """
        if isinstance(root_or_paths, list):
            self.paths = sorted(root_or_paths)
        elif isinstance(root_or_paths, str):
            # If a string, treat it as folder and collect image paths
            self.paths = sorted(data_utils.make_dataset(root_or_paths))
        else:
            raise ValueError("Expected a list of paths or a directory string for 'root_or_paths'.")
...
class ImageDatasetFlexible(Dataset):
    def __init__(self, root, transform=None, image_root=None):

        self.image_root = "/home/ids/yuhe/Projects/CA_with_GAN/2_data/styleGAN/ffhq_1024"
        self.transform = transform

        if root.endswith(".npy"):
            assert self.image_root is not None, "image_root must be provided when using a .npy file"
            self.using_npy = True
            rel_paths = np.load(root, allow_pickle=True).tolist()
            self.paths = [os.path.join(self.image_root, p) for p in rel_paths]
        else:
            self.using_npy = False
            self.paths = sorted(data_utils.make_dataset(root))
