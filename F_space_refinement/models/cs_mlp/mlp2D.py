...
class MappingNetwork_cs_2Dmlp(nn.Module):
    """
    Mapping network for StyleGAN latents shaped [B, 18, 512], 
    where all 18 rows are equivalent. 
    This maps the first row to `C` and `S`, then repeats the result.
    """
    ...
    def forward(self, z: torch.Tensor):  # z: [B, 18, 512]
        z_single = z[:, 0, :]                 # [B, 512]
        c = self.net_c(z_single)             # [B, 512]
        s = self.net_s(z_single)             # [B, 512]
        c = c.unsqueeze(1).repeat(1, self.style_dim, 1)  # [B, 18, 512]
        s = s.unsqueeze(1).repeat(1, self.style_dim, 1)  # [B, 18, 512]
        return c, s
