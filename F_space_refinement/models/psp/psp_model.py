...
class pSp(nn.Module):
	...
	def load_weights(self):
		if self.previous_train_ckpt is not None:
			print('Loading pSp from previous train checkpoint: {}'.format(self.opts.exp_dir))
			#ckpt = torch.load(self.previous_train_ckpt, map_location='cpu', weights_only=True)
			ckpt=self.previous_train_ckpt
			self.encoder.load_state_dict(get_keys(ckpt, 'encoder'), strict=True)
			self.decoder.load_state_dict(get_keys(ckpt, 'decoder'), strict=True)
			self.__load_latent_avg(ckpt)
		elif self.opts.pSp_checkpoint_path is not None:
			...
			encoder_ckpt = torch.load(DefaultPaths['ir_se50_path'], weights_only=True)
	...
	def forward(self, x, resize=True, latent_mask=None, input_code=False, randomize_noise=True,
				...
				recon_modle=False, return_codes=False, encode_only=False):
		if input_code:
			codes = x
		else:
		...
		if encode_only:
			...
			if return_latents and return_codes:
				return images, codes_pSp, result_latent
			if return_latents:
