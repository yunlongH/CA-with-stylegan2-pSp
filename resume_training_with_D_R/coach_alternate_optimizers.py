...
class Coach_csmlp:
	def __init__(self, opts, previous_train_ckpt=None):
		...
		self.optimizer_CA, self.optimizer_R  = self.configure_optimizers()
	...
	def	init_disc_network(self, prev_train_path):
		self.disc_model = CustomLatentClassifier(num_layers=2).to(self.device)
	...
	def calc_contrastiveR_loss(self, latent_bg_c, latent_t_c, latent_t_s):
		...
		if self.opts.recon_loss_type == 'mse':
			loss = F.mse_loss(recon_t_s, torch.zeros_like(latent_t_s))
		elif self.opts.recon_loss_type == 'l1':
			loss = F.l1_loss(recon_t_s, torch.zeros_like(latent_t_s))
		...
		with torch.no_grad():
			if self.opts.recon_loss_type == 'mse':
				...
				d_loss_bg = F.l1_loss(recon_bg_s, torch.zeros_like(recon_bg_s))
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_t_dataloader)):
				...
				loss_ca = loss_lat + loss_img_bg + loss_img_t + loss_cls

				# Backpropagate separately for each optimizer
				loss_ca.backward(retain_graph=True)  # Compute gradients for loss1
				self.optimizer_CA.step()  # Update with lr=0.01
				
				loss_recon.backward()  # Compute gradients for loss2
				self.optimizer_R.step()  # Update with lr=0.001
	...
	def configure_optimizers(self):
		...
		if self.opts.optim_name == 'adam':
			optimizer_CA = torch.optim.Adam(params, lr=self.opts.lr_CA)
		else:
			optimizer_CA = Ranger(params, lr=self.opts.lr_CA)
		optimizer_R = torch.optim.Adam(params, lr=self.opts.lr_R)

		return optimizer_CA, optimizer_R
