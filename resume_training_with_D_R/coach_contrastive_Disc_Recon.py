...
from models.mlp3D import MappingNetwork_cs_independent, EqualizedLinear, MappingNetwork_c2s
...
class Coach_csmlp:
	def __init__(self, opts, previous_train_ckpt=None):
		...
		# Initialize network
		self.init_cmlp_networks(prev_train_path = self.opts.cmlp_checkpoint_path)
		self.init_disc_network(prev_train_path = self.opts.disc_checkpoint_path)
		self.init_c2s_network(prev_train_path = self.opts.c2s_checkpoint_path)
	...
	def init_cmlp_networks(self, prev_train_path):
		# pSp_ckpt = None
		# cs_ckpt = None
		print(f'Loading cs encoder from previous checkpoint...{prev_train_path}')
		prev_train_ckpt = torch.load(prev_train_path, map_location='cpu', weights_only=True)
		# opts = prev_train_ckpt['opts']
		# opts = Namespace(**opts)
		cs_ckpt  = prev_train_ckpt['state_dict_cs_enc']	
		self.global_step = prev_train_ckpt['global_step']

		self.pSp_net = pSp(self.opts, previous_train_ckpt=None).to(self.device).eval()
		self.cs_mlp_net = MappingNetwork_cs_independent(self.opts).to(self.device)	

		print(f'Resuming training from weights step {self.global_step}')
		self.cs_mlp_net.load_state_dict(cs_ckpt)
		
	def	init_disc_network(self, prev_train_path):
		self.disc_model = CustomLatentClassifier(num_layers=self.opts.num_D_layers).to(self.device)
		print(f"Loaded model from {prev_train_path}")
		self.disc_model.load_state_dict(torch.load(prev_train_path, map_location=self.device))
		self.disc_model.eval()  # Set to evaluation mode

	def	init_c2s_network(self, prev_train_path):
		if self.opts.c2s_net_type == 'simple':
			self.c2s_model = SimpleLinearModel().to(self.device)
		elif self.opts.c2s_net_type == 'deep':
			self.c2s_model = DeepC2SModel().to(self.device)
		elif self.opts.c2s_net_type == 'strong': 
			self.c2s_model = StrongerC2SModel().to(self.device)
		elif self.opts.c2s_net_type == 'c2smlp': 
			self.c2s_model = MappingNetwork_c2s(self.opts).to(self.device)

		print(f"Loaded model from {prev_train_path}")
		self.c2s_model.load_state_dict(torch.load(prev_train_path, map_location=self.device))
		self.c2s_model.eval()  # Set to evaluation mode

	def calc_cls_loss(self, latent_bg_c, latent_t_c):

		latent_bg_c = latent_bg_c.view(latent_bg_c.shape[0], -1)  # (4, 9216)
		latent_t_c = latent_t_c.view(latent_t_c.shape[0], -1)  # (4, 9216)

		logits_bg = self.disc_model(latent_bg_c)  # Raw outputs (logits) for background latents
		logits_t = self.disc_model(latent_t_c)  # Raw outputs (logits) for target latents

		loss = self.criterion(logits_t, torch.zeros_like(logits_t))  # Target = 1

		with torch.no_grad():
			loss_d_bg = self.criterion(logits_bg, torch.zeros_like(logits_bg))
			loss_d_t = self.criterion(logits_t, torch.ones_like(logits_t))
			probs_bg = torch.sigmoid(logits_bg)
			probs_t = torch.sigmoid(logits_t)

		loss_dict = loss.item()

		log_msg = (f"Step [{int(self.global_step)}/{int(self.opts.max_steps)}] | "
				f"G_loss_ct: {loss_dict:.4f}, D_loss_cbg: {loss_d_bg.item():.4f}, D_loss_ct: {loss_d_t.item():.4f} | "
				f"probs_cbg: {probs_bg.mean().item():.4f}, probs_ct: {probs_t.mean().item():.4f}\n")

		return loss  * self.opts.w_cls_lambda, log_msg

	# def calc_contrastiveR_loss(self, latent_bg_c, latent_t_c, latent_t_s):
	# 	recon_t_s = self.c2s_model(latent_t_c)
	# 	recon_bg_s = self.c2s_model(latent_bg_c)

	# 	if self.opts.recon_loss_type == 'mse':
	# 		loss = F.mse_loss(recon_t_s, recon_bg_s.detach())
	# 	elif self.opts.recon_loss_type == 'l1':
	# 		loss = F.l1_loss(recon_t_s, recon_bg_s.detach())

	# 	with torch.no_grad():
	# 		if self.opts.recon_loss_type == 'mse':
	# 			d_loss_bg = F.mse_loss(recon_bg_s, recon_bg_s.detach())
	# 			d_loss_t = F.mse_loss(recon_t_s, latent_t_s)
	# 		elif self.opts.recon_loss_type == 'l1':
	# 			d_loss_bg = F.l1_loss(recon_bg_s, recon_bg_s.detach())
	# 			d_loss_t = F.l1_loss(recon_t_s, latent_t_s)
		
	# 	loss_dict = loss.item()

	# 	# Logging before optimizer step
	# 	log_msg = (f"Step [{int(self.global_step)}/{int(self.opts.max_steps)}] | "
	# 			f"loss_t_inv: {loss_dict:.4f}, d_loss_bg: {d_loss_bg:.4f}, d_loss_t: {d_loss_t:.4f} \n")

	# 	return loss * self.opts.w_recon_lambda, log_msg 

	def calc_contrastiveR_loss(self, latent_bg_c, latent_t_c, latent_t_s):

		# lambda_s = 0.1  # Weight for keeping net(c_t) far from s_t

		# Compute outputs from self.c2s_model (your net)
		recon_t_s = self.c2s_model(latent_t_c)  # net(c_t)
		recon_bg_s = self.c2s_model(latent_bg_c)  # net(c_bg)

		# 1. Minimize net(c_t) (force it close to zero)
		#loss_close_to_zero = F.mse_loss(recon_t_s, torch.zeros_like(recon_t_s))
		loss_close_to_zero = F.mse_loss(recon_t_s, recon_bg_s.detach())

		# 2. Keep net(c_t) far from s_t
		loss_far_from_s = -self.opts.lambda_s * F.mse_loss(recon_t_s, latent_t_s)

		# Final inversion loss
		loss_inversion = loss_close_to_zero + loss_far_from_s

		with torch.no_grad():
				d_loss_bg = F.mse_loss(recon_bg_s, torch.zeros_like(recon_bg_s))
				d_loss_t = F.mse_loss(recon_t_s, latent_t_s)
		
		loss_dict = loss_inversion.item()

		# Logging before optimizer step
		log_msg = (f"Step [{int(self.global_step)}/{int(self.opts.max_steps)}] | "
				f"loss_t_inv: {loss_dict:.4f}, loss_close_to_zero: {loss_close_to_zero:.4f}, loss_far_from_s: {loss_far_from_s:.4f}, g d_loss_bg: {d_loss_bg:.4f}, d_loss_t: {d_loss_t:.4f} \n")

		return loss_inversion * self.opts.w_recon_lambda, log_msg 
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_t_dataloader)):
				...
				# Compute classification loss (adversarial)
				loss_cls, cls_log_msg = self.calc_cls_loss(latent_bg_c, latent_t_c)
				# Compute recon loss 
				loss_recon, recon_log_msg  = self.calc_contrastiveR_loss(latent_bg_c, latent_t_c, latent_t_s)
				...
				loss = loss_lat + loss_img_bg + loss_img_t + loss_recon + loss_cls

				loss.backward()
				self.optimizer.step()
				
				# # Logging related
				if self.global_step % self.opts.log_interval == 0 or (self.global_step < 500 and self.global_step % 25 == 0):
					self.write_log_to_txt(cls_log_msg, "train_loss_cls.txt")
					self.write_log_to_txt(recon_log_msg, "train_loss_recon.txt")
				...
				if self.global_step % self.opts.val_interval == 0 or self.global_step == self.opts.max_steps:
					val_loss_dict, val_cls_log_msg, val_recon_log_msg = self.validate()
					self.write_log_to_txt(val_cls_log_msg, "val_loss_cls.txt")
					self.write_log_to_txt(val_recon_log_msg, "val_loss_recon.txt")
	...
	def validate(self):
		...
		self.cs_mlp_net.train()
		return loss_dict, cls_log_msg, recon_log_msg
