...
from training_classifier.cls_models import CustomLatentClassifier
...
class Coach_csmlp:
	def __init__(self, opts, previous_train_ckpt=None):
		...
		# Initialize network
		self.init_networks(prev_train_path = self.opts.cmlp_checkpoint_path)
		self.init_cls_network(prev_train_path = self.opts.cls_checkpoint_path)
		self.criterion = nn.BCEWithLogitsLoss()  # Use the same loss function as classifier training
	...
	def init_networks(self, prev_train_path):
		# pSp_ckpt = None
		# cs_ckpt = None
	...
	def	init_cls_network(self, prev_train_path):

		self.cls_model = CustomLatentClassifier(input_dim=18*512, num_layers=2, hidden_dim=None).to(self.device)
		# Load weights
		print(f"Loaded model from {prev_train_path}")
		self.cls_model.load_state_dict(torch.load(prev_train_path, map_location=self.device))
		self.cls_model.eval()  # Set to evaluation mode
	...
	def calc_cls_loss(self, latent_bg_c, latent_t_c):
		...
		logits_bg = self.cls_model(latent_bg_c)  # Raw outputs (logits) for background latents
		logits_t = self.cls_model(latent_t_c)  # Raw outputs (logits) for target latents

		loss_cls = self.criterion(logits_t, torch.zeros_like(logits_t))  # Target = 1
		...
		return loss_cls, loss_d_bg, loss_d_t, probs_bg, probs_t
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_t_dataloader)):
				...
				loss_cls, loss_d_bg, loss_d_t, probs_bg, probs_t = self.calc_cls_loss(latent_bg_c, latent_t_c)
				
				log_msg = (f"Step [{int(self.global_step)}/{int(self.opts.max_steps)}] | "
						f"G_loss_ct: {loss_cls.item():.4f}, D_loss_cbg: {loss_d_bg.item():.4f}, D_loss_ct: {loss_d_t.item():.4f} | "
				...
				loss = loss_cls * self.opts.w_cls_lambda + loss_lat + loss_img_bg + loss_img_t
				...
				if self.global_step % self.opts.log_interval == 0 or (self.global_step < 500 and self.global_step % 25 == 0):
					self.write_log_to_txt(log_msg, "train_cls_loss.txt")
				...
				if self.global_step % self.opts.val_interval == 0 or self.global_step == self.opts.max_steps:
					val_loss_dict, log_msg = self.validate()
					self.write_log_to_txt(log_msg, "val_cls_loss.txt")
	...
	def validate(self):
		...
		# Initialize accumulators for losses and probabilities
		total_loss_cls = 0
		total_loss_d_bg = 0
		total_loss_d_t = 0
		total_probs_bg = 0
		...
		for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.test_bg_dataloader, self.test_t_dataloader)):
			...
			with torch.no_grad():
				...
				# Accumulate losses and probabilities
				total_loss_cls += loss_cls.item()
				total_loss_d_bg += loss_d_bg.item()
				total_loss_d_t += loss_d_t.item()
				total_probs_bg += probs_bg.mean().item()
				total_probs_t += probs_t.mean().item()
		...
		# Ensure all values are standard Python floats
		avg_loss_cls = float(total_loss_cls / num_batches)
		avg_loss_d_bg = float(total_loss_d_bg / num_batches)
		avg_loss_d_t = float(total_loss_d_t / num_batches)
		avg_probs_bg = float(total_probs_bg / num_batches)
		avg_probs_t = float(total_probs_t / num_batches)

		log_msg = (f"Step [{int(self.global_step)}/{int(self.opts.max_steps)}] | "
				f"G_loss_ct: {avg_loss_cls:.4f}, D_loss_cbg: {avg_loss_d_bg:.4f}, D_loss_ct: {avg_loss_d_t:.4f} | "
				f"probs_cbg: {avg_probs_bg:.4f}, probs_ct: {avg_probs_t:.4f}\n")
