...
class Coach_csmlp:
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_glass_dataloader)):
				...
				rec_x_bg = self.pSp_net.forward(latent_bg, input_code=True, randomize_noise=True, recon_modle=True)
				rec_x_t = self.pSp_net.forward(latent_t, input_code=True, randomize_noise=True, recon_modle=True)	

				# Calculate loss 
				loss_lat, loss_lat_dict = self.calc_latent_loss(latent_bg_s, latent_t_s, latent_bg, latent_t, w_bg_pSp, w_t_pSp)
	...
	def validate(self):
		...
		for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.test_bg_dataloader, self.test_glass_dataloader)):
			...
			with torch.no_grad():
				...
				# Calculate loss 
				_, loss_lat_dict = self.calc_latent_loss(latent_bg_s, latent_t_s, latent_bg, latent_t, w_bg_pSp, w_t_pSp)
	...
	def L1_lasso_output_regularization(self, output, direction='element', reg_type='traditional', threshold=0.0, epsilon=1e-8):
		"""
		Apply different types of L1 regularization (Traditional, Logarithmic, Adaptive, Thresholded) on the output tensor.
		...
		Args:
			...
			reg_type (str): Type of regularization ('traditional', 'logarithmic', 'adaptive', 'thresholded').
		...
		if direction == 'row':
			# Apply regularization across rows
			return torch.sum(regularization_fn(output).sum(dim=2))
		elif direction == 'column':
			# Apply regularization across columns
			return torch.sum(regularization_fn(output).sum(dim=1))
		elif direction == 'element':
			# Apply regularization to individual elements
			return torch.sum(regularization_fn(output))
		else:
			raise ValueError(f"Invalid direction: '{direction}'.")

	def scad_mcp_output_regularization(self, output, direction='element', reg_type='SCAD', lambda_reg=0.1, a=3.7, gamma=3):
		"""
		Apply SCAD or MCP regularization on the output tensor.

		Args:
			output (torch.Tensor): The tensor on which to apply regularization.
			direction (str): The direction for regularization ('row', 'column', or 'element').
			reg_type (str): Type of regularization ('SCAD' or 'MCP').
			lambda_reg (float): Regularization strength (threshold for shrinkage).
			a (float): SCAD parameter controlling the threshold for minimal penalization.
			gamma (float): MCP parameter controlling the threshold for minimal penalization.
		...
		if reg_type == 'SCAD':
			...
			def scad_penalty(x):
				...
				# Case 2: lambda_reg < |x| <= a * lambda_reg
		...
		# Apply regularization function based on direction
		if direction == 'row':
			# Regularization across rows
			return torch.sum(regularization_fn(output).sum(dim=1))
		elif direction == 'column':
			# Regularization across columns
			return torch.sum(regularization_fn(output).sum(dim=0))
		elif direction == 'element':
			# Regularization on individual elements
			return torch.sum(regularization_fn(output))
		else:
			raise ValueError(f"Invalid direction: '{direction}'.")
	...
	def calc_latent_loss(self, latent_bg_s, latent_t_s, latent_bg, latent_t, latent_bg_target, latent_t_target):
		"""
		...
		if self.opts.lasso_sbg_lambda > 0:
			lasso_sbg_loss = self.L1_lasso_output_regularization(latent_bg_s, direction=self.opts.lasso_direction, reg_type=self.opts.lasso_reg_type, threshold=self.opts.lasso_threshold)
		...
		if self.opts.lasso_st_lambda > 0:
			lasso_st_loss = self.L1_lasso_output_regularization(latent_t_s, direction=self.opts.lasso_direction, reg_type=self.opts.lasso_reg_type, threshold=self.opts.lasso_threshold)
			loss_dict['lasso_st_loss'] = float(lasso_st_loss)
			loss += lasso_st_loss * self.opts.lasso_st_lambda

		with torch.no_grad():	
			zero_count_bg = (latent_bg_s.abs() <= 0.0).sum().item()	
			zero_count_t = (latent_t_s.abs() <= 0.0).sum().item()
