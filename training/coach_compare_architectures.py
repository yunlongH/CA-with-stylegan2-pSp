...
from models.mlp3D import MappingNetwork_cs_independent, EqualizedLinear, \
	MappingNetwork_cs_shared, FactorizationNetwork_cs, MappingNetwork_cs_independent_spatial, \
	MappingNetwork_cs_independent_ResNet, MappingNetwork_cs_hierarchical, CNNMappingNetwork
...
class Coach_csmlp:
	...
	def init_networks(self, prev_train_ckpt):
		...
		if self.opts.net_type == 'independent':
			print('using independent MLP')
			self.cs_mlp_net = MappingNetwork_cs_independent(self.opts).to(self.device)	
		elif self.opts.net_type == 'shared':
			print('using shared MLP')
			self.cs_mlp_net = MappingNetwork_cs_shared(self.opts).to(self.device)	
		elif self.opts.net_type == 'factorization': 
			print('using Factorization network')
			self.cs_mlp_net = FactorizationNetwork_cs(self.opts).to(self.device)	
		elif self.opts.net_type == 'spatial-encoding':
			self.cs_mlp_net = MappingNetwork_cs_independent_spatial(self.opts).to(self.device)
		elif self.opts.net_type == 'hierarchical':
			self.cs_mlp_net = MappingNetwork_cs_hierarchical(self.opts).to(self.device)
		elif self.opts.net_type == 'simple_cnn':
			self.cs_mlp_net = CNNMappingNetwork(self.opts).to(self.device)
		elif self.opts.net_type == 'independent_ResNet':
			self.cs_mlp_net = MappingNetwork_cs_independent_ResNet(self.opts).to(self.device)
		else:
			raise ValueError(f"Invalid network type: '{self.opts.net_type}'.")
