...
class Coach_csmlp:
	...
	def	init_c2s_network(self, prev_train_path):
		...
		self.recon_model = MappingNetwork_c2s(self.opts).to(self.device)
		# Load weights
		print(f"Loaded model from {prev_train_path}")
		self.recon_model.load_state_dict(torch.load(prev_train_path, map_location=self.device))
		self.recon_model.eval()  # Set to evaluation mode
	...
	def calc_contrastiveR_loss(self, latent_bg_c, latent_t_c, latent_t_s):
		...
		recon_t_s = self.recon_model(latent_t_c)
		recon_bg_s = self.recon_model(latent_bg_c)

		if self.opts.c2s_loss_type == 'mse':
			loss = F.mse_loss(recon_t_s, torch.zeros_like(latent_t_s))
		elif self.opts.c2s_loss_type == 'l1':
		...
		return loss * self.opts.w_c2s_lambda, log_msg 
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_t_dataloader)):
				...
				# Compute recon loss 
				loss_contrastive, train_log_msg  = self.calc_contrastiveR_loss(latent_bg_c, latent_t_c, latent_t_s)
				...
				loss = loss_lat + loss_img_bg + loss_img_t + loss_contrastive 
				...
				if self.global_step % self.opts.log_interval == 0 or (self.global_step < 500 and self.global_step % 25 == 0):
					self.write_log_to_txt(train_log_msg, "train_loss_contrastive.txt")
				...
				if self.global_step % self.opts.val_interval == 0 or self.global_step == self.opts.max_steps:
					val_loss_dict, val_log_msg = self.validate()
					self.write_log_to_txt(val_log_msg, "val_loss_contrastive.txt")
	...
	def validate(self):
		...
		for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.test_bg_dataloader, self.test_t_dataloader)):
			...
			with torch.no_grad():
				...
				# Compute recon loss 
				loss_contrastive, log_msg  = self.calc_contrastiveR_loss(latent_bg_c, latent_t_c, latent_t_s)
