...
class Coach_csmlp:
	def __init__(self, opts, previous_train_ckpt=None):
		...
		self.optimizer, self.optimizerD = self.configure_optimizers()
	...
	def init_discriminator(self, disc_ckpt_path):
		...
		if disc_ckpt_path is not None and os.path.exists(disc_ckpt_path):
			print(f'Resuming training discriminator from path: {disc_ckpt_path}')
	...
	def update_discriminator(self, x_bg, x_t):
		...
		D_out_real = output_real.mean().item()

		# Forward pass fake batch through D
		output_fake = self.discriminator(latent_t_c.detach()).view(-1)
		errD_fake = self.criterion(output_fake, fake_label)
		D_out_fake = output_fake.mean().item()

		# Compute total loss and update Discriminator
		errD = (errD_real + errD_fake) * self.opts.w_adv_lambda
		errD.backward()
		self.optimizerD.step()

		return errD_real, errD_fake, D_out_real, D_out_fake

		
	def calc_gen_loss(self, w_t_pSp, latent_t_c):

		# Generate fake labels
		b_size = w_t_pSp.size(0)
		real_label = torch.ones(b_size, device=latent_t_c.device)  # Generator wants D to think it's real

		# Forward pass fake batch through D
		output_fake = self.discriminator(latent_t_c).view(-1)
		
		# Compute generator loss
		errG = self.criterion(output_fake, real_label)

		G_out_fake = output_fake.mean().item()

		return errG, G_out_fake
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_t_dataloader)):
				...
				if self.opts.train_D_common: 
					errD_real, errD_fake, D_out_real, D_out_fake = self.update_discriminator(x_bg, x_t)
				...
				errG_fake, G_out_fake = self.calc_gen_loss(w_t_pSp, latent_t_c)
				loss_adv = errG_fake * self.opts.w_adv_lambda
				...
				loss = loss_adv + loss_lat + loss_img_bg + loss_img_t

				loss.backward()
				self.optimizer.step()

				
				if self.global_step % self.opts.log_interval == 0 or (self.n_adv_print < 200 and self.global_step % 50 == 0):
					if self.opts.train_D_common: 
						log_msg = ('[%d/%d]\tLoss_D real: %.4f Loss_D fake: %.4f\tLoss_G: %.4f\tD(x): %.4f\tD(G(z)): %.4f / %.4f\n'
									% (self.global_step, self.opts.max_steps, errD_real.item(), errD_fake.item(), errG_fake.item(), \
										D_out_real, D_out_fake, G_out_fake))
					else:
						log_msg = ('[%d/%d]\tLoss_G: %.4f\tD(G(z)): %.4f\n'
									% (self.global_step, self.opts.max_steps, errG_fake.item(), G_out_fake))						
	...
	def configure_optimizers(self):
		...
		params_disc = list(self.discriminator.parameters())
		optimizerD = torch.optim.Adam(params_disc, lr=self.opts.D_learning_rate)
