...
class Coach_csmlp:
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_t_dataloader)):
				...
				w_bg_pSp = self.pSp_net.forward(x_bg, encode_only=True)
				w_t_pSp = self.pSp_net.forward(x_t, encode_only=True) 
				...
				rec_x_bg = self.pSp_net.forward(latent_bg_c, input_code=True, randomize_noise=False, recon_modle=True)
				rec_x_t = self.pSp_net.forward(latent_t_c + latent_t_s, input_code=True, randomize_noise=False, recon_modle=True)	
				
				swap_x_bg = self.pSp_net.forward(latent_bg_c + latent_t_s, input_code=True, randomize_noise=False, recon_modle=True)
				swap_x_t = self.pSp_net.forward(latent_t_c , input_code=True, randomize_noise=False, recon_modle=True)	
	
				swap_w_bg_pSp = self.pSp_net.forward(swap_x_bg, encode_only=True)
				swap_w_t_pSp = self.pSp_net.forward(swap_x_t, encode_only=True) 

				if self.opts.w_dist_rec_lambda > 0:
					rec_bg_c, rec_t_s = self.cs_mlp_net(swap_w_bg_pSp, zero_out_silent=self.opts.zero_out_silent_bg)
					rec_t_c, rec_bg_s = self.cs_mlp_net(swap_w_t_pSp, zero_out_silent=self.opts.zero_out_silent_t)
				else:
					rec_bg_c, rec_t_s, rec_t_c, rec_bg_s = 0, 0, 0, 0
					
				# Calculate loss
				loss_lat, loss_lat_dict = self.calc_latent_loss(latent_bg_c, latent_bg_s, latent_t_c, latent_t_s, w_bg_pSp, w_t_pSp, \
											rec_bg_c, rec_bg_s, rec_t_c, rec_t_s, swap_w_bg_pSp, swap_w_t_pSp)
	...
	def calc_latent_loss(self, latent_bg_c, latent_bg_s, latent_t_c, latent_t_s, w_bg_pSp, w_t_pSp, \
		...
		if self.opts.sbg_lambda > 0:
			loss_sbg = F.mse_loss(latent_bg_s, torch.zeros_like(latent_bg_s).to(self.device))
			loss_dict['loss_sbg'] = float(loss_sbg)
			loss += loss_sbg * self.opts.sbg_lambda

		if self.opts.w_dist_lambda > 0:
			loss_dist_bg = F.mse_loss(latent_bg_c, w_bg_pSp)
			loss_dist_t = F.mse_loss(latent_t_c + latent_t_s, w_t_pSp)
			loss_dict['loss_dist_bg'] = float(loss_dist_bg)
			loss_dict['loss_dist_t'] = float(loss_dist_t)
			loss += (loss_dist_bg + loss_dist_t) * self.opts.w_dist_lambda	

		if self.opts.w_dist_swap_lambda > 0:
			loss_swap_dist_bg = F.mse_loss(latent_bg_c + latent_t_s, swap_w_bg_pSp)
			loss_swap_dist_t = F.mse_loss(latent_t_c, swap_w_t_pSp)
			loss_dict['loss_swap_dist_bg'] = float(loss_swap_dist_bg)
			loss_dict['loss_swap_dist_t'] = float(loss_swap_dist_t)
			loss += (loss_swap_dist_bg + loss_swap_dist_t) * self.opts.w_dist_swap_lambda	

		if self.opts.w_dist_rec_lambda > 0:
			loss_rec_cbg = F.mse_loss(latent_bg_c, rec_bg_c)
			loss_rec_sbg = F.mse_loss(torch.zeros_like(rec_bg_s).to(self.device), rec_bg_s)
			loss_rec_ct = F.mse_loss(latent_t_c, rec_t_c)
			loss_rec_st = F.mse_loss(latent_t_s, rec_t_s)
			loss_dict['loss_rec_cbg'] = float(loss_rec_cbg)
			loss_dict['loss_rec_sbg'] = float(loss_rec_sbg)
			loss_dict['loss_rec_ct'] = float(loss_rec_ct)
			loss_dict['loss_rec_st'] = float(loss_rec_st)
			loss += (loss_rec_cbg + loss_rec_sbg + loss_rec_ct + loss_rec_st) * self.opts.w_dist_rec_lambda	
