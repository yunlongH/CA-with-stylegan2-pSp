...
class Coach_csmlp:
	def __init__(self, opts, previous_train_ckpt=None):
		...
		# Initialize network
		self.load_pSp_cmlp_networks(model_path=self.opts.cmlp_checkpoint_path)
		self.load_c2s_network(model_path=self.opts.c2s_checkpoint_path)
		...
		self.optimizer, self.optimizer_c2s = self.configure_optimizers()
	...
	def load_pSp_cmlp_networks(self, model_path):
		...
		self.cmlp_net = MappingNetwork_cs_independent(model_opts).to(self.device)
		print(f"Resuming cmlp training from step {model_ckpt['global_step']}")  
		self.cmlp_net.load_state_dict(model_ckpt['state_dict_cs_enc'])

	def load_c2s_network(self, model_path):

		c2s_ckpt = torch.load(model_path, map_location='cpu', weights_only=True)
		c2s_opts = c2s_ckpt['opts']
		c2s_opts = Namespace(**c2s_opts)

		self.c2s_net = MappingNetwork_c2s(c2s_opts).to(self.device)
		print(f"Resuming c2s_net training from {model_path}") 
		self.c2s_net.load_state_dict(c2s_ckpt['state_dict_c2s_mlp'])
	...
	def pSp_csmlp_encoding(self, images):
		...
		c, s = self.cmlp_net(w_pSp, zero_out_silent=self.opts.zero_out_silent_t) 

		return c, s

	def update_c2s_mlp(self, x_t):

		self.c2s_net.train()  # Set model to training mode
		self.optimizer_c2s.zero_grad()  # Clear optimizer gradients

		# Forward pass
		c_t, s_t = self.pSp_csmlp_encoding(x_t)
		s_t_recon = self.c2s_net(c_t)
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_t_dataloader)):
				...
				if self.opts.train_c2s_mlp and self.global_step % self.opts.train_c2s_interval == 0 :
					self.update_c2s_mlp(x_t)

				_, w_bg_pSp = self.pSp_net.forward(x_bg, return_latents=True)
				_, w_t_pSp = self.pSp_net.forward(x_t, return_latents=True) 

				latent_bg_c, latent_bg_s = self.cmlp_net(w_bg_pSp, zero_out_silent=self.opts.zero_out_silent_bg)
				latent_t_c, latent_t_s = self.cmlp_net(w_t_pSp, zero_out_silent=self.opts.zero_out_silent_t) 

				s_t_recon = self.c2s_net(latent_t_c)
				...
				# Calculate loss
				loss_lat, loss_lat_dict = self.calc_latent_loss(latent_bg_c, latent_bg_s, latent_t_c, latent_t_s, s_t_recon.detach(), w_bg_pSp, w_t_pSp)
				...
				if self.global_step < 500 and self.global_step % 25 == 0 :
	...
	def validate(self):
		...
		for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.test_bg_dataloader, self.test_t_dataloader)):
			...
			with torch.no_grad():
				...
				# Calculate loss
				_, loss_lat_dict = self.calc_latent_loss(latent_bg_c, latent_bg_s, latent_t_c, latent_t_s, s_t_recon, w_bg_pSp, w_t_pSp)
	...
	def configure_optimizers(self):
		params = list(self.cmlp_net.parameters())
		params_c2s = list(self.c2s_net.parameters())
		...
		optimizer_c2s = torch.optim.Adam(params_c2s, lr=self.opts.c2s_learning_rate)
		return optimizer, optimizer_c2s
	...
	def calc_latent_loss(self, latent_bg_c, latent_bg_s, latent_t_c, latent_t_s, s_t_recon, w_bg_pSp, w_t_pSp):
		...
		if self.opts.w_adv_lambda > 0:
			loss_adv_g = -F.mse_loss(s_t_recon, latent_t_s)  # Maximize this
			loss_dict['loss_adv_g'] = float(loss_adv_g)
			loss += loss_adv_g * self.opts.w_adv_lambda	
		...
		# 	lasso_weight_loss = (self.elastic_net_regularization(self.cmlp_net, network='net_s', reg_type=self.opts.lasso_weight_type, alpha=self.opts.elastic_alpha) 
		# 		if self.cmlp_net else 0.0)
	...
	def merge_loss_dict(self, loss_lat_dict, loss_img_dict_bg, loss_img_dict_t):
		...
		loss_sum = loss_lat_dict['loss'] + loss_img_dict_bg['loss'] + loss_img_dict_t['loss']

		# If 'loss_adv_g' exists in loss_lat_dict, subtract it
		if 'loss_adv_g' in loss_lat_dict:
			loss_sum -= loss_lat_dict['loss_adv_g']		
		loss_dict['loss_sum'] = loss_sum  # Store adjusted loss sum
	...
	def __get_save_dict(self):
		save_dict = {
			...
			'state_dict_cs_enc': self.cmlp_net.state_dict(),
			'opts': vars(self.opts)
		}
