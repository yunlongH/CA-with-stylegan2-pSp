...
class Coach_csmlp:
	def __init__(self, opts, previous_train_ckpt=None):
		...
		# Initialize network
		self.init_networks(cmlp_checkpoint_path=self.opts.cmlp_checkpoint_path)
		self.load_pretrained_mi_disc(mi_disc_checkpoint_path =self.opts.mi_disc_checkpoint_path)
	...
	def init_networks(self, cmlp_checkpoint_path):

		print(f'Loading pSp and cs encoder from previous checkpoint: \n{cmlp_checkpoint_path}') 
		model_ckpt = torch.load(cmlp_checkpoint_path, map_location='cpu', weights_only=True)
		model_opts = model_ckpt['opts']
		model_opts = Namespace(**model_opts)

		self.global_step = model_ckpt['global_step'] + 1
		self.pSp_net = pSp(model_opts).to(self.device).eval()

		self.cs_mlp_net = MappingNetwork_cs_independent(model_opts).to(self.device)	
		print(f'loading cs_mlp pretrained weights, step-{self.global_step}')
		self.cs_mlp_net.load_state_dict(model_ckpt['state_dict_cs_enc'])

	def load_pretrained_mi_disc(self, mi_disc_checkpoint_path):

		print(f"loading mi_disc from {mi_disc_checkpoint_path}") 
		disc_ckpt = torch.load(mi_disc_checkpoint_path, map_location='cpu', weights_only=True)
		disc_opts = disc_ckpt['opts']
		disc_opts = Namespace(**disc_opts)

		if disc_opts.disc_type == 'global':
			self.mi_discriminator = DiscriminatorGlobal(disc_opts).to(self.opts.device)
		else:
			self.mi_discriminator = Discriminator(disc_opts).to(self.opts.device)

		self.mi_discriminator.load_state_dict(disc_ckpt['state_dict_mi_disc'])
		self.mi_discriminator.eval()
	...
	def calc_mi_loss(self, c, s):
		
		# Compute D_lambda(h, d)
		D_output = self.mi_discriminator(c, s)  # Probability from trained discriminator

		# Avoid numerical instability
		eps = 1e-10
		D_output = torch.clamp(D_output, eps, 1 - eps)

		# Compute MI loss
		mi_loss = torch.relu(torch.log(D_output / (1 - D_output))).sum()

		return mi_loss

	def calc_mi_loss_logit(self, c, s):
		
		# Compute D_lambda(h, d)
		D_output = self.mi_discriminator(c, s)  # Probability from trained discriminator

		# Compute logit transformation (log(D / (1 - D)))
		logits = torch.logit(D_output, eps=1e-10)  # eps avoids numerical instability

		# Apply ReLU to ensure non-negative values
		mi_loss = torch.relu(logits).sum()  # Mean over batch
	...
	def calc_latent_loss(self, latent_bg_c, latent_bg_s, latent_t_c, latent_t_s, w_bg_pSp, w_t_pSp):
		...
		if self.opts.w_mi_lambda > 0:
			if self.opts.mi_loss_type == 'logit':
				loss_mi = self.calc_mi_loss_logit(latent_t_c, latent_t_s)
			else:
				loss_mi = self.calc_mi_loss(latent_t_c, latent_t_s)
				
			loss_dict['loss_mi'] = float(loss_mi)
			loss += loss_mi * self.opts.w_mi_lambda	
	...
	def merge_loss_dict(self, loss_lat_dict, loss_img_dict_bg, loss_img_dict_t):
		...
		if 'loss_mi' in loss_lat_dict:
			loss_sum -= loss_lat_dict['loss_mi']		
