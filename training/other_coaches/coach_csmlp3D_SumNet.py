...
from models.mlp3D import MappingNetwork_cs_SumNet, EqualizedLinear 
...
class Coach_csmlp:
	...
	def init_networks(self, prev_train_ckpt):
		...
		self.cs_mlp_net = MappingNetwork_cs_SumNet(self.opts).to(self.device)	
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_glass_dataloader)):
				...
				latent_bg, latent_bg_c, latent_bg_s = self.cs_mlp_net(w_bg_pSp, zero_out_silent=self.opts.zero_out_silent_bg, zero_sbg=True)
				latent_t, latent_t_c, latent_t_s = self.cs_mlp_net(w_t_pSp, zero_out_silent=self.opts.zero_out_silent_t) 
