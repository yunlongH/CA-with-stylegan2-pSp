...
from models.mlp3D import MappingNetwork_cs_independent, MappingNetwork_c2s, EqualizedLinear
...
class Coach_csmlp:
	...
	def init_networks(self, prev_train_ckpt):
		...
		if prev_train_ckpt is not None:
			...
			c2s_ckpt  = prev_train_ckpt['state_dict_c2s_enc']	
		...
		if c2s_ckpt is not None:
			print('Loading c2s encoder from previous checkpoint...')
			self.cs_mlp_net.load_state_dict(c2s_ckpt)
	...
	def set_model_frozen(self, model, frozen=True):
		"""
		Freeze or unfreeze the parameters of a given module.
		
		Args:
			model (torch.nn.Module): The model to freeze or unfreeze.
			frozen (bool): If True, freeze the parameters. If False, unfreeze them.
		"""
		for param in model.parameters():
			param.requires_grad = not frozen

	def train_c2s_mlp(self, x_t):
		
		self.c2s_mlp.train()
		# self.cs_mlp_net.eval()
		self.set_model_frozen(self.cs_mlp_net, frozen=True)
		self.set_model_frozen(self.c2s_mlp, frozen=False)
		

		self.optimizer_c2s.zero_grad()
		...
		c, s = self.cs_mlp_net(w_t_pSp, zero_out_silent=self.opts.zero_out_silent_t) 

		s_recon = self.c2s_mlp(c)

		loss_adv_d = F.mse_loss(s, s_recon)
		loss_adv_d_dict = float(loss_adv_d)

		loss_adv_d.backward()
		self.optimizer_c2s.step()

		if self.global_step % self.opts.c2s_interval == 0 or (self.global_step < 1000 and self.global_step % 25 == 0):
			self.write_metrics_to_txt(loss_adv_d_dict, prefix='train', filename='loss_adv_d_for_check.txt')

		self.set_model_frozen(self.cs_mlp_net, frozen=False)
		self.set_model_frozen(self.c2s_mlp, frozen=True)
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_t_dataloader)):
				...
				self.train_c2s_mlp(x_t.to(self.device).float())
				torch.cuda.empty_cache()
				...
				if self.global_step % 25 == 0 and self.n_print < 100:
					...
					self.n_print += 1
				if self.global_step % self.opts.c2s_interval == 0 or (self.global_step < 1000 and self.global_step % 25 == 0):
					self.write_metrics_to_txt(loss_lat_dict['adv_c2s'], prefix='train', filename='loss_adv_g_for_check.txt')
	...
	def configure_optimizers(self):
		...
		params_c2s = list(self.c2s_mlp.parameters())
		optimizer_c2s = torch.optim.Adam(params_c2s, lr=self.opts.learning_rate)
	...
	def calc_latent_loss(self, latent_bg_c, latent_bg_s, latent_t_c, latent_t_s, latent_bg_target, latent_t_target):
		...
		if self.opts.adv_c2s_lambda > 0:
				latent_t_s_recon = self.c2s_mlp(latent_t_c)
				loss_c2s = - F.mse_loss(latent_t_s, latent_t_s_recon)
				loss_dict['adv_c2s'] = float(loss_c2s)
				loss += float(loss_c2s) * self.opts.w_dist_lambda
