...
class Coach_csmlp:
	def __init__(self, opts, previous_train_ckpt=None):
		...
		# Initialize dataset
		self.train_bg_dataset, self.train_glass_dataset, self.test_bg_dataset, self.test_glass_dataset = self.configure_datasets()
		...
		self.train_glass_dataloader = DataLoader(self.train_glass_dataset,
									batch_size=self.opts.batch_size,
									shuffle=True,
									num_workers=int(self.opts.workers),
									drop_last=True)
		...
		self.test_glass_dataloader = DataLoader(self.test_glass_dataset,
										  batch_size=self.opts.test_batch_size,
										  shuffle=False,
										  num_workers=int(self.opts.test_workers),
										  drop_last=True)		
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_glass_dataloader)):
				...
				if self.global_step < 100 and self.global_step % 25 == 0 :
	...
	def validate(self):
		...
		# 	x, y = batch
		for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.test_bg_dataloader, self.test_glass_dataloader)):
	...
	def configure_datasets(self):
		...
		train_glass_dataset = ImagesDataset(source_root=dataset_args['train_glass_source_root'],
									  target_root=dataset_args['train_glass_target_root'],
									  source_transform=transforms_dict['transform_source'],
									  target_transform=transforms_dict['transform_gt_train'],
									  opts=self.opts)
		...
		test_glass_dataset = ImagesDataset(source_root=dataset_args['test_glass_source_root'],
									 target_root=dataset_args['test_glass_target_root'],
									 source_transform=transforms_dict['transform_source'],
									 target_transform=transforms_dict['transform_test'],
									 opts=self.opts)
		...
		if self.opts.use_wandb:
			...
			self.wb_logger.log_dataset_wandb(train_glass_dataset, dataset_name="Train_glass")
			self.wb_logger.log_dataset_wandb(test_bg_dataset, dataset_name="Test_bg")
			self.wb_logger.log_dataset_wandb(test_glass_dataset, dataset_name="Test_glass")

		print(f"Number of training samples: {len(train_bg_dataset)}")
		print(f"Number of test samples: {len(test_bg_dataset)}")
		return train_bg_dataset, train_glass_dataset, test_bg_dataset, test_glass_dataset
	...
	def output_regularization(self, output, direction='element', reg_type='traditional', lambda_reg=0.1, a=3.7, gamma=3, threshold=0.0, epsilon=1e-8):
		"""
		Apply different types of regularization (Traditional, Logarithmic, Adaptive, Thresholded, SCAD, MCP) on the output tensor.

		Args:
			output (torch.Tensor): The tensor on which to apply regularization.
			direction (str): The direction for regularization ('row', 'column', or 'element').
			reg_type (str): Type of regularization ('traditional', 'logarithmic', 'adaptive', 'thresholded', 'SCAD', 'MCP').
			lambda_reg (float): Regularization strength (threshold for shrinkage).
			a (float): SCAD parameter controlling the threshold for minimal penalization (only used if reg_type='SCAD').
			gamma (float): MCP parameter controlling the threshold for minimal penalization (only used if reg_type='MCP').
			threshold (float): Threshold for Thresholded L1 regularization (only used if reg_type='thresholded').
			epsilon (float): Small constant to prevent division by zero in Adaptive L1 regularization.
	...
	def Directional_alignment_loss(self, C, S, WpSp, type="background", dim=1):
		"""
		Calculate directional alignment loss for style tensors, 
		depending on whether the input is background or target.

		Args:
			C (torch.Tensor): Tensor for C (batchsize, 18, 512).
			S (torch.Tensor): Tensor for S (batchsize, 18, 512).
			WpSp (torch.Tensor): Tensor for WpSp (batchsize, 18, 512).
			type (str): Either "background" or "target".
			dim (int): Dimension along which to normalize and compute cosine similarity (default=2).

		Returns:
			torch.Tensor: Scalar value representing the directional alignment loss.
		"""
		if dim not in [1, 2]:
			raise ValueError("dim must be 1 (style layers) or 2 (latent dimensions).")

		if type == "background":
			# Background: Force C_bg = WpSp_bg
			C_direction = F.normalize(C, dim=dim)
			WpSp_direction = F.normalize(WpSp, dim=dim)
			cosine_similarity = F.cosine_similarity(C_direction, WpSp_direction, dim=dim)
			directional_alignment_loss = 1 - torch.mean(cosine_similarity)

		elif type == "target":
			# Target: Force C_t + S_t = WpSp_t
			C_S_direction = F.normalize(C + S, dim=dim)
			WpSp_direction = F.normalize(WpSp, dim=dim)
			cosine_similarity = F.cosine_similarity(C_S_direction, WpSp_direction, dim=dim)
			directional_alignment_loss = 1 - torch.mean(cosine_similarity)

		else:
			raise ValueError("Invalid type. Must be 'background' or 'target'.")

		return directional_alignment_loss

	
	def Orthogonality_loss(self, C, S):
		"""
		Calculate orthogonality loss to enforce C and S are orthogonal.
		...
		Returns:
			torch.Tensor: Scalar value representing the orthogonality loss.
		"""
		# Compute the dot product along the feature dimension
		dot_product = torch.sum(C * S, dim=2)  # Shape: (batchsize, 18)
		orthogonality_loss = torch.mean(torch.abs(dot_product))  # Mean across batch and layers

		return orthogonality_loss


		#loss_dict['orthogonality_loss_t'] = float(orthogonality_loss)

	def calc_latent_loss(self, latent_bg_c, latent_bg_s, latent_t_c, latent_t_s, latent_bg_target, latent_t_target):
		...
		if self.opts.w_dist_lambda > 0:
			loss_distance_bg = F.mse_loss(latent_bg_c, latent_bg_target)
			loss_distance_t = F.mse_loss(latent_t_c + latent_t_s, latent_t_target)
		...
		if self.opts.directional_align_lambda > 0:
			loss_dir_align_bg = self.Directional_alignment_loss(latent_bg_c, latent_bg_s, latent_bg_target, type='background', dim=self.opts.directional_align_dim)
			loss_dir_align_t = self.Directional_alignment_loss(latent_t_c, latent_t_s, latent_t_target, type='target', dim=self.opts.directional_align_dim)
			loss_dict['loss_dir_align_bg'] = float(loss_dir_align_bg)
			loss_dict['loss_dir_align_t'] = float(loss_dir_align_t)
			loss += (loss_dir_align_bg + loss_dir_align_t) * self.opts.directional_align_lambda

		if self.opts.orthogonality_lambda > 0:
			orthogonality_loss = self.Orthogonality_loss(latent_t_c, latent_t_s)
			loss_dict['orthogonality_loss'] = float(orthogonality_loss)
			loss += orthogonality_loss * self.opts.orthogonality_lambda
