...
class Coach_csmlp:
	...
	def init_networks(self, prev_train_ckpt):
		"""
		Initialize networks and optionally load from a previous training checkpoint.
		"""
		pSp_ckpt = None
		cs_ckpt = None
		ema_ckpt = None
		...
		if prev_train_ckpt is not None:
			...
			ema_ckpt = prev_train_ckpt['state_dict_cs_enc_ema']  # Correct key for EMA model
		...
		self.cs_mlp_net_ema = copy.deepcopy(self.cs_mlp_net)
		for param in self.cs_mlp_net_ema.parameters():
			param.requires_grad = False

		# Load cs_mlp_net and sync EMA if checkpoint exists
		...
		if ema_ckpt is not None:
			print('Loading EMA model from previous checkpoint...')
			self.cs_mlp_net_ema.load_state_dict(ema_ckpt)  # Correct key for EMA model
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_glass_dataloader)):
				...
				# Update EMA model
				self.accumulate(self.cs_mlp_net_ema, self.cs_mlp_net, decay=0.999)	
				...
				if self.global_step % self.opts.val_interval == 0 or self.global_step == self.opts.max_steps:
					val_loss_dict = self.validate(use_ema=self.opts.use_ema)
	...
	def validate(self, use_ema=True):
		...
		if use_ema:
			model_to_use = self.cs_mlp_net_ema
		else:
			model_to_use = self.cs_mlp_net
		...
		for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.test_bg_dataloader, self.test_glass_dataloader)):
			...
			with torch.no_grad():
				...
				latent_bg_c, latent_bg_s = model_to_use(w_bg_pSp, zero_out_silent=self.opts.zero_out_silent_bg)
				latent_t_c, latent_t_s = model_to_use(w_t_pSp, zero_out_silent=self.opts.zero_out_silent_t) 
	...
	def __get_save_dict(self):
		save_dict = {
			...
			'cs_mlp_net': self.cs_mlp_net.state_dict(),
			'cs_mlp_net_ema': self.cs_mlp_net_ema.state_dict(),
			'opts': vars(self.opts),
