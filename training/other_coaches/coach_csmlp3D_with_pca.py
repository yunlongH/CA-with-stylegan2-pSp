...
class Coach_csmlp:
	...
	def compute_PCA_weights(self, from_latent, on_latent, norm_type="exp", weight_axis="columns", scale_factor=10):
		...
		for i in range(batch_size):
			...
			sample_from_latent = from_latent[i].detach().cpu().numpy()
		...
		return weighted_latent * scale_factor 
	...
	def calc_latent_loss(self, latent_bg_s, latent_t_s, latent_bg, latent_t, w_bg_pSp, w_t_pSp):
		...
		weighted_latent_bg = self.compute_PCA_weights(w_bg_pSp, latent_bg, 
								norm_type=self.opts.pca_norm_type, weight_axis=self.opts.pca_weight_axis, scale_factor=self.opts.scale_factor)
		weighted_w_bg_pSp = self.compute_PCA_weights(w_bg_pSp, w_bg_pSp, 
								norm_type=self.opts.pca_norm_type, weight_axis=self.opts.pca_weight_axis, scale_factor=self.opts.scale_factor)		
		
		weighted_latent_t = self.compute_PCA_weights(w_t_pSp, latent_t, 
								norm_type=self.opts.pca_norm_type, weight_axis=self.opts.pca_weight_axis, scale_factor=self.opts.scale_factor)
		weighted_w_t_pSp = self.compute_PCA_weights(w_t_pSp, w_t_pSp, 
								norm_type=self.opts.pca_norm_type, weight_axis=self.opts.pca_weight_axis, scale_factor=self.opts.scale_factor)
		
		if self.opts.w_dist_lambda > 0:
			loss_distance_bg = F.mse_loss(weighted_latent_bg, weighted_w_bg_pSp)
			loss_distance_t = F.mse_loss(weighted_latent_t, weighted_w_t_pSp)
