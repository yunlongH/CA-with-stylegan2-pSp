...
class Coach_csmlp:
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_glass_dataloader)):
				...
				if self.global_step % 25 == 0 and self.n_print < 5:
	...
	def L1_lasso_output_regularization(self, output, reg_type='row'):
		if reg_type == 'row':
	...
	def L1_lasso_weight_regularization(self, model, network='net_s', reg_type='all'):
		"""
		...
		# Apply L1 regularization based on reg_type
		if reg_type == 'all':
			# Apply Lasso to all EqualizedLinear layers in the target network
			for layer in layers:
				if isinstance(layer[0], EqualizedLinear):
					# Check weight tensor dimensions to ensure compatibility with both 2D and 3D cases
					lasso_weight_penalty += torch.sum(torch.abs(layer[0].weight.weight))  # Applies to both 2D and 3D
		elif reg_type == 'last':
			...
			if isinstance(last_layer, EqualizedLinear):
				lasso_weight_penalty += torch.sum(torch.abs(last_layer.weight.weight))  # Applies to both 2D and 3D
		else:
	...
	def calc_latent_loss(self, latent_bg_s, latent_t_s, latent_bg, latent_t, latent_bg_target, latent_t_target):
		...
		if self.opts.lasso_sbg_lambda > 0:
			lasso_sbg_loss = self.L1_lasso_output_regularization(latent_bg_s, reg_type=self.opts.lasso_output_type)
			loss_dict['lasso_sbg_loss'] = float(lasso_sbg_loss)
			loss += lasso_sbg_loss * self.opts.lasso_sbg_lambda

		if self.opts.lasso_st_lambda > 0:
			lasso_st_loss = self.L1_lasso_output_regularization(latent_t_s, reg_type=self.opts.lasso_output_type)
		...
		if self.opts.lasso_weight_lambda > 0:
			lasso_weight_loss = (self.L1_lasso_weight_regularization(self.cs_mlp_net, network='net_s', reg_type=self.opts.lasso_weight_type) 
