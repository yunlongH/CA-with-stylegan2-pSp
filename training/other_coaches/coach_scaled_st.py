...
class Coach_csmlp:
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, (batch_bg, batch_t) in enumerate(zip(self.train_bg_dataloader, self.train_glass_dataloader)):
				...
				norm_w_bg_pSp = self.center_with_avg(w_bg_pSp)
				norm_w_t_pSp = self.center_with_avg(w_t_pSp)

				latent_bg_c, latent_bg_s = self.cs_mlp_net(norm_w_bg_pSp, zero_out_silent=self.opts.zero_out_silent_bg)
				latent_t_c, latent_t_s = self.cs_mlp_net(norm_w_t_pSp, zero_out_silent=self.opts.zero_out_silent_t) 

				#latent_bg_s = self.opts.s_scale_lambda * math.sqrt(512)*F.normalize(latent_bg_s, p=2, dim=2)
				latent_t_s = self.opts.s_scale_lambda * math.sqrt(512)*F.normalize(latent_t_s, p=2, dim=2)

				latent_bg_rec = self.shift_with_avg(latent_bg_c)
				latent_t_rec = self.shift_with_avg(latent_t_c + latent_t_s)

				rec_x_bg = self.pSp_net.forward(latent_bg_rec, input_code=True, randomize_noise=True, recon_modle=True)
				rec_x_t = self.pSp_net.forward(latent_t_rec, input_code=True, randomize_noise=True, recon_modle=True)	

				# Calculate loss 
				loss_lat, loss_lat_dict = self.calc_latent_loss(latent_bg_c, latent_bg_s, latent_t_c, latent_t_s, norm_w_bg_pSp, norm_w_t_pSp)
	...
	def calc_latent_loss(self, latent_bg_c, latent_bg_s, latent_t_c, latent_t_s, norm_w_bg_pSp, norm_w_t_pSp):
		...
		if self.opts.st_lambda > 0:
			# Compute L2 norm for each 1x512 vector of latent_t_s
			loss_silent_t = ((torch.norm(latent_t_s, p=2, dim=2) - 1.0) ** 2).mean()
			loss_dict['loss_silent_t'] = float(loss_silent_t)
			loss += loss_silent_t * self.opts.st_lambda

		if self.opts.w_dist_lambda > 0:
			latent_bg = latent_bg_c 
			latent_t = latent_t_c + latent_t_s
			loss_distance_bg = F.mse_loss(latent_bg, norm_w_bg_pSp)
			loss_distance_t = F.mse_loss(latent_t, norm_w_t_pSp)
